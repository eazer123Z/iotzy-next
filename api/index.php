 ...
 * Path: api/index.php
 * MODIF: Tambah CORS headers untuk React frontend
 */

// ── CORS Headers ──────────────────────────────────────────────
$allowedOrigins = [
    getenv('FRONTEND_URL') ?: 'https://iotzy.vercel.app',
    'http://localhost:5173',
    'http://localhost:3000',
];
...
if ($action) {
    ...
    $db        = getLocalDB();
    if (!$db) jsonOut(['success' => false, 'error' => 'Database unreachable.'], 500);
...
// ── Tidak ada action → tolak (React yg handle UI sekarang) ────
header('Content-Type: application/json');
